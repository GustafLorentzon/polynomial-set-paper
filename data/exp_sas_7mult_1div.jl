using LinearAlgebra
@inline function exp_sas_7mult_1div(A)
    T=promote_type(eltype(A),Rational{Int64})
    A_copy=similar(A,T); copyto!(A_copy, A);
    return exp_sas_7mult_1div!(A_copy)
end

@inline function exp_sas_7mult_1div!(A)
    T=promote_type(eltype(A),Rational{Int64}) # Make it work for many 'bigger' types (matrices and scalars)
    # max_memslots=8
    n=size(A,1)
    ...
    # Computation order: C A2 A4 A6 Ua Va Ub3 Ub Uc U Vb3 Vb V Z X P S1
